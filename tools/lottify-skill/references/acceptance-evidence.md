...
The manifest schema is enforced by the checker. Production acceptance must set `acceptance.level: production-go`; local checkpoints and milestone acceptance cannot satisfy a production release gate. Record decisions, handoffs, checkpoints and durable evidence references under `records` so the Lead's claim is auditable.
