...
## Reusable project-agent core and next-action decisions

Lottify is the first project profile for the reusable `../project-agent/` core. Lottify product requirements, domain routing, review subskills, Ticket 16 evidence and Ticket 19 sequencing remain authoritative in this skill.
...
Use `tools/project-agent/scripts/next_action.py` when a structured action state is available. The output contract is: Current State -> Gap -> Recommended Action -> Owner -> Lottify subskills -> Runtime skills -> Expected Result -> Required Evidence. If no action is executable, return the actual wait/block state and resume condition; do not invent work.

Runtime/Matt engineering skills are selected through `tools/project-agent/skills/runtime-skill-packs.yaml` and recorded in the Lottify execution manifest. They provide implementation/review technique only and cannot override requirements, ownership, domain invariants or acceptance gates. Specification/ticket-generation runtime skills require an explicit Change Request.

On a concrete Hermes/Codex host, use the runtime inventory generated by `tools/project-agent/scripts/install_runtime.py` when resolving the next action. Skills missing from that runtime are reported as unavailable and must not be impersonated.
...
Newly generated manifest-v2 records include `project-agent` and deterministic runtime-skill routing. Existing manifest-v2 records that predate this integration remain readable; regeneration opts them into the new routing checks.
