...
## Issue #24 Admin UI/E2E governed publication acceptance — local checkpoint (2026-09-16)

Source: Tickets 05/10/16 and the approved Admin configuration-publish UX flow. This checkpoint closes the previously listed local Admin journey evidence gap for the scoped Product publication lifecycle; it does not by itself close Issue #24 or establish Production GO.

- Added a root Playwright harness using the installed Google Chrome runtime and deterministic browser-level Admin API mocks. The success journey proves the user-visible Product version lifecycle `DRAFT → REVIEW → PUBLISHED`, action-scoped MFA re-auth (`lottery-configuration.publish`), stable command idempotency keys, and the immutable-published-state guidance shown to the operator. A full-page published-state screenshot is attached to the Playwright HTML report as supplemental visual evidence.
- Added a negative maker-checker journey. A REVIEW version receives successful MFA re-auth, the approve command returns `MAKER_CHECKER_VIOLATION`, the UI exposes the governed error/correlation reference, no success state is shown, and the authoritative mocked version remains REVIEW. This supplements the existing HTTP integration evidence rather than replacing backend separation-of-duties tests.
- The browser journey found a real Admin Lottery bootstrap defect: `LotteryWorkspace.initialize()` passed `{}` to `AdminApi.request("auth/me", {})`, which made the shared client issue `POST /api/v1/admin/auth/me` even though the approved/backend contract is `GET /api/v1/admin/auth/me`. The Lottery workspace now calls `request("auth/me")` without a body. No backend API contract, schema, migration or domain rule changed.
- Local result on Node `v24.20.0` with `@playwright/test 1.63.0`: Admin Lottery E2E `2/2` passed; general Vitest `436 passed / 235 skipped`; `pnpm typecheck` passed; `pnpm build:admin` passed. The skipped integration suites are unchanged default gating and this local checkpoint does not substitute for immutable-candidate CI.
- Remaining for this post-UI candidate: independent review of the UI/E2E diff and full CI/fresh candidate verification because the 2026-09-09 immutable-candidate CI predates this UI fix and Playwright dependency. Issue #24 remains open until those applicable gates are satisfied.
