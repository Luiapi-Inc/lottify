...
1. Accounting-period/effective-time enforcement, including rejection of postings/backdating into CLOSED periods and current-period correction linkage.
2. Recovery/debt behavior that can represent negative net position without exposing negative value as spendable `CASH`, with betting/withdrawal availability clamped to zero while unresolved according to policy.
