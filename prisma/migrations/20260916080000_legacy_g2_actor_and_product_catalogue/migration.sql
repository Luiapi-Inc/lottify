...
CREATE TABLE "lottery_number_limits" (
    ...
    -- The resolution invariant: a DRAW-scoped limit must name its Draw and may not
    -- be a TYPE-scoped one; a limit that names a Draw must name its Bet Type too,
    -- because a Draw carries several Bet Types. G2's five GLOBAL rows carry a
    -- lottery_type_id (a per-product instance of one G3 Bet Type code) and no
    -- draw_id, and its single DRAW row carries both, so the migrated set satisfies
    -- this exactly.
    CONSTRAINT "lottery_number_limits_scope_target_check" CHECK (
        ("scope" <> 'DRAW' OR "draw_id" IS NOT NULL)
        AND ("scope" <> 'TYPE' OR "draw_id" IS NULL)
