...
import { type DepositMethodDescription, type DepositMethodSummary, MemberApiFailure, memberApi } from "../../lib/member-api";
...
export default function DepositPage() {
  ...
  const confirm = async () => {
    if (!selectedMethod || !description || !canSubmit) return;
    setSubmitting(true);
    setError(null);
    try {
  ...
  return <main id="main">
    ...
    </div><aside className="stack"><section className="summary-box"><div className="summary-row"><span>ช่องทาง</span><strong>{selectedMethod ? methodLabel(selectedMethod.methodCode) : "-"}</strong></div><div className="summary-row"><span>จำนวนฝาก</span><strong>{formatBaht(amountMinor)} บาท</strong></div><div className="summary-row"><span>ค่าธรรมเนียม</span><strong>{loadingDescription ? "กำลังตรวจสอบ" : description ? `${formatBaht(feeMinor)} บาท` : "-"}</strong></div><div className="summary-row total"><span>ยอดชำระ</span><strong>{description ? `${formatBaht(totalMinor)} บาท` : "-"}</strong></div><button className={`button lime block ${canSubmit ? "" : "disabled"}`} type="button" disabled={!canSubmit} aria-disabled={!canSubmit} onClick={confirm} style={{ marginTop: 12 }}>{submitting ? "กำลังสร้างรายการ..." : "ยืนยันและรับคำแนะนำ →"}</button></section><div className="notice info"><b>i</b><div><strong>อย่าสร้างรายการซ้ำหากยังรอตรวจสอบ</strong>สถานะ Pending จะอัปเดตรายการเดิมเมื่อผู้ให้บริการยืนยัน</div></div></aside></section>
  </main>;
}
